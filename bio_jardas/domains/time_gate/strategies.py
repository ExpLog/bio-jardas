...
class TimeGateStrategy(ABC):
    ...
    @abstractmethod
    def lock_gate(
        self, time_gate: "TimeGate", action_time: ZonedDateTime
    ) -> "TimeGate":
...
class FortuneTellerTimeGateStrategy(
    ...
    def lock_gate(
        ...
        date = action_time.date()
        days_until_monday = (7 - date.day_of_week().value + 1) % 7

        candidate = action_time.add(days=days_until_monday).replace_time(Time(hour=7))
        if candidate <= ZonedDateTime.now_in_system_tz():
